...
const chart = new Chart(ctx,{type:'line',data:data,options:{animation:{duration:300},responsive:true,plugins:{legend:{position:'top'}},scales:{x:{display:true},y:{display:true}}}});
...
function animateSensorIcons(){
  ...
  icons.forEach((el, idx)=>{
    ...
    // add pulse for warning/critical simulation
...
function updateRobotVisual(){
  ...
  if(!robot) return;
  // Map distance (10..500) to percent (10%..80%)
  ...
  // breadcrumb path: place a fading dot at robot center
  if(pathEl){
...
pauseBtn.addEventListener('click',()=>{streaming=!streaming;pauseBtn.textContent = streaming? '⏸':'▶';});
...
document.getElementById('clearAlerts').addEventListener('click',()=>alertsList.innerHTML='');
